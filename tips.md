...
Images generated by this tool already store the prompt in PNG `parameters` — `caption` reads that instantly (use `--force-caption` to run BLIP). `--embed-prompt` writes the extracted prompt back into the source file. Multiple images: `caption a.png b.png`.
